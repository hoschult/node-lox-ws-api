...
Connection.prototype.register_client = function () {
    ...
    this._ws.client.on('connectFailed', function (error) {
        console.log("WebSocket error: ", this._ws.connection.closeDescription, this._ws.connection.closeReasonCode);
        let reason = that._ws.connection.closeDescription + ' (' + that._ws.connection.closeReasonCode + ')';
...
Connection.prototype.register_connection = function (connection) {
    ...
    */
    ...
    this._ws.connection.on('close', function (_reason, _desc) {
        console.log("Websocket Connection closed: ", _reason, _desc);
        let reason = _desc + ' (' + _reason + ')';
        that._ws.connection = undefined;
        clearInterval(that._keepalive_interval);
        clearInterval(that._reconnect_interval);
        if (that._connection_data.last_header && that._connection_data.last_header.identifier == '5') {
            that.emit('close', true, reason);
        } else {
            that.emit('close', false, reason);
        }
    });
    ...
    this._ws.connection.on('error', function (error) {
        ...
        clearInterval(that._keepalive_interval);
        clearInterval(that._reconnect_interval);
        let reason = this._ws.connection.closeDescription + ' (' + this._ws.connection.closeReasonCode + ')';
